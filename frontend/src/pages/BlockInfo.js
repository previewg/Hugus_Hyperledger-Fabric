...
const BlockInfo = (props) => {
  ...
  const blockList = useSelector((state) => state.block.blockList.list);
  const blockStatus = useSelector((state) => state.block.blockList.status);
  const [data, setData] = useState([]);

  const Socket = () => {
    useEffect(() => {
      const socket = socketIOClient("http://192.168.0.22:2002");
      socket.on("hugus", (res) => {
